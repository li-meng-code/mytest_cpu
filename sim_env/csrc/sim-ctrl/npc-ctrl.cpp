...
#ifdef CONFIG_WAVE_TRACE
static void wave_dump(unsigned int step) {
...
void npc_exec_once(Decode *s) {
  ...
  sim_commit_context(&s->pc, &s->dnpc, &s->isa.inst);
  single_cycle(); /* write back and fetch */
}
...
void init_npc(int argc, char *argv[], const char *wavefile) {
  ...
  npc->trace(tfp.get(), 99); // the tracing level is set to 4
