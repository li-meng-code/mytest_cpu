...
static uint8_t pmem [CONFIG_MSIZE] PG_ALIGN = { }; /* psram */
static uint8_t flash [CONFIG_FLASH_SIZE] PG_ALIGN = { 0x78, 0x56, 0x34, 0x12 , 0x12, 0x34, 0x56, 0x78 }; /* magic number to test */
...
uint8_t *guest_to_host(paddr_t paddr) {
  ...
  if (likely(in_mrom(paddr))) return mrom + paddr - CONFIG_MROM_BASE;
  if (likely(in_flash(paddr))) return flash + paddr - CONFIG_FLASH_BASE;
  if (likely(in_sdram(paddr))) return sdram + paddr - CONFIG_SDRAM_BASE;
  if (likely(in_vmem(paddr))) return vmem + paddr - CONFIG_VMEM_BASE;
  assert(0);
}

paddr_t host_to_guest(uint8_t *haddr) { 
  if (likely(haddr - pmem < CONFIG_MSIZE)) return haddr - pmem + CONFIG_MBASE;
  if (likely(haddr - mrom < CONFIG_MROM_SIZE)) return haddr - mrom + CONFIG_MROM_BASE;
  if (likely(haddr - flash < CONFIG_FLASH_SIZE)) return haddr - flash + CONFIG_FLASH_BASE;
  if (likely(haddr - sdram < CONFIG_SDRAM_SIZE)) return haddr - sdram + CONFIG_SDRAM_BASE;
  if (likely(haddr - vmem < CONFIG_VMEM_SIZE)) return haddr - vmem + CONFIG_VMEM_BASE;
  assert(0);
}
...
word_t paddr_read(paddr_t addr, int len) {
  if (likely(in_storage(addr))) { /* 0x8000_0000 - 0x8fff_ffff */
    return pmem_read(addr, len);
  }
/********************************************************************** 
 * MMIO is used for virtual devices; if an address also maps to a hard
 * device, the request is handled by the hard device via the xbar and
 * does not fall back to MMIO.
 **********************************************************************/
#ifdef CONFIG_DEVICE
